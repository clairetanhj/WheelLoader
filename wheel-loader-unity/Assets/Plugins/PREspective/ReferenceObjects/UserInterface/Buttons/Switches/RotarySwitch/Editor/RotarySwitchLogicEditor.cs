...
namespace u040.prespective.referenceobjects.userinterface.buttons.switches.editor
{
    ...
    [CustomEditor(typeof(RotarySwitchLogic))]
    public class RotarySwitchLogicEditor : PrespectiveEditor
    {
        private RotarySwitchLogic component;
        ...
        {
            component = (RotarySwitchLogic)target;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (component.RotarySwitch == null)
            {
                EditorGUILayout.HelpBox("No RotarySwitch has been set. This component will not function properly untill all required components have been assigned. You can do this in the Properties tab.", MessageType.Warning);
            }
            ...
            switch (toolbarTab.intValue)
            {
                case 0:
                    if (Application.isPlaying) //Make sure motor physical properties cannot be editted during playmode
                    {
                        if (component.RotarySwitch == null)
                        {
                            EditorGUILayout.LabelField("No RotarySwitch has been set so no properties can be shown.");
                        }
                        else
                        {
                            EditorGUILayout.LabelField("Rotary Switch", component.RotarySwitch.ToString());
                        }
                    }
                    else
                    {
                        component.RotarySwitch = (RotarySwitch)EditorGUILayout.ObjectField("Rotary Switch", component.RotarySwitch, typeof(RotarySwitch), true);
                    }
                    break;
