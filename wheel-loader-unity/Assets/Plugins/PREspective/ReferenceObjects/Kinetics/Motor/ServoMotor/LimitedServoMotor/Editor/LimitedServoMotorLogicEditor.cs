...
namespace u040.prespective.referenceobjects.kinetics.motor.servomotor.editor
{
    ...
    [CustomEditor(typeof(LimitedServoMotorLogic))]
    public class LimitedServoMotorLogicEditor : PrespectiveEditor
    {
        private LimitedServoMotorLogic component;
        ...
        {
            component = (LimitedServoMotorLogic)target;
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                EditorGUILayout.HelpBox("No LimitedServoMotor Component has been set. This component will not function properly untill all required components have been assigned. You can do this in the Properties tab.", MessageType.Warning);
            }
            ...
            switch (toolbarTab.intValue)
            {
                case 0:
                    if (Application.isPlaying) //Make sure motor physical properties cannot be editted during playmode
                    {
                        if (component.ServoMotor == null)
                        {
                            EditorGUILayout.LabelField("No LimitedServoMotor has been set so no properties can be shown.");
                        }
                        else
                        {
                            EditorGUILayout.LabelField("LimitedServoMotor", component.ServoMotor.ToString());
                        }
                    }
                    else
                    {
                        component.ServoMotor = (LimitedServoMotor)EditorGUILayout.ObjectField("LimitedServoMotor", component.ServoMotor, typeof(LimitedServoMotor), true);
                    }
                    break;
