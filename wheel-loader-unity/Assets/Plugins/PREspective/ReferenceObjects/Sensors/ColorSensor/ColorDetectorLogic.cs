...
{
    public class ColorDetectorLogic : PreLogicComponent
    {
...
#pragma warning restore 0414
        ...
        void onSignalChanged(SignalInstance _signal, object _newValue, DateTime _newValueReceived, object _oldValue, DateTime _oldValueReceived)
        {
            switch (_signal.definition.defaultSignalName)
            {
                case "oActive":
                    ...
                    ColorDetector.IsActive = oActive;
                    break;
        ...
        {
            if (ColorDetector.IsActive != iActive)
            {
                iActive = ColorDetector.IsActive;
                WriteValue("iActive", iActive);
            }

            if (ColorDetector.OutputSignal != iSensorOutput)
            {
                iSensorOutput = ColorDetector.OutputSignal;
