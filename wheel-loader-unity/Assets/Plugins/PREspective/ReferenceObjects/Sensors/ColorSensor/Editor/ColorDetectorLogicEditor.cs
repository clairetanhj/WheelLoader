...
namespace u040.prespective.referenceobjects.sensors.colorsensor.editor
{
    ...
    [CustomEditor(typeof(ColorDetectorLogic))]
    public class ColorDetectorLogicEditor : PrespectiveEditor
    {
        private ColorDetectorLogic component;
        ...
        {
            component = (ColorDetectorLogic)target;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (component.ColorDetector == null)
            {
                EditorGUILayout.HelpBox("No ColorDetector has been set. This component will not function properly untill all required components have been assigned. You can do this in the Properties tab.", MessageType.Warning);
            }
            ...
            switch (toolbarTab.intValue)
            {
                case 0:
                    if (Application.isPlaying) //Make sure motor physical properties cannot be editted during playmode
                    {
                        if (component.ColorDetector == null)
                        {
                            EditorGUILayout.LabelField("No ColorDetector has been set so no properties can be shown.");
                        }
                        else
                        {
                            EditorGUILayout.LabelField("ColorDetector", component.ColorDetector.ToString());
                        }
                    }
                    else
                    {
                        component.ColorDetector = (ColorDetector)EditorGUILayout.ObjectField("ColorDetector", component.ColorDetector, typeof(ColorDetector), true);
                    }
                    break;
