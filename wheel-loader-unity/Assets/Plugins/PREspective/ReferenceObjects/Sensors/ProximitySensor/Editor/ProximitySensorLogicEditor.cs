...
namespace u040.prespective.referenceobjects.sensors.proximitysensor.editor
{
    ...
    [CustomEditor(typeof(ProximitySensorLogic))]
    public class ProximitySensorLogicEditor : PrespectiveEditor
    {
        private ProximitySensorLogic component;
        ...
        {
            component = (ProximitySensorLogic)target;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (component.ProximitySensor == null)
            {
                EditorGUILayout.HelpBox("No ProximitySensor has been set. This component will not function properly untill all required components have been assigned. You can do this in the Properties tab.", MessageType.Warning);
            }
            ...
            switch (toolbarTab.intValue)
            {
                case 0:
                    if (Application.isPlaying) //Make sure motor physical properties cannot be editted during playmode
                    {
                        if (component.ProximitySensor == null)
                        {
                            EditorGUILayout.LabelField("No ProximitySensor has been set so no properties can be shown.");
                        }
                        else
                        {
                            EditorGUILayout.LabelField("ProximitySensor", component.ProximitySensor.ToString());
                        }
                    }
                    else
                    {
                        component.ProximitySensor = (ProximitySensor)EditorGUILayout.ObjectField("ProximitySensor", component.ProximitySensor, typeof(ProximitySensor), true);
                    }
                    break;
